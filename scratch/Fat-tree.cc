...
main (int argc, char *argv[])
{
  ...
  NS_LOG_INFO("  tcp_adu_size=" << tcp_adu_size);
  ...
  for (i = 0; i < num_flows; i++) // for (i = 0; i < total_host; i++)
    {
      ...
      if (UdpEchoTestOnly)
        ...
        { // full duration test;
          app[i] = ftp.Install (node); // using oo. or ftp.
  ...
  if (flow_monitor)
    {
      ...
      monitor->SerializeToXmlFile (prefix_file_name + ".k" + ".xml", true, true);
    }
